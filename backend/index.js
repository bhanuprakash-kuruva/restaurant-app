    ...
    <body>
      <div class="container">
        ...
        <div class="status-card">
          <h3>Server Status</h3>
          <p><strong>API Base URL:</strong> ${req.protocol}://${req.get('host')}</p>
          <p><strong>Environment:</strong> <span class="badge badge-info">${process.env.NODE_ENV || 'development'}</span></p>
          <p><strong>Frontend URL:</strong> ${process.env.FRONTEND_URL || 'Not set'}</p>
          <p><strong>MongoDB:</strong> <span class="badge ${MONGO_URI ? 'badge-success' : 'badge-warning'}">${MONGO_URI ? '✅ Connected' : '❌ Not Configured'}</span></p>
...
.catch((err) => {
  ...
  setTimeout(() => {
    mongoose.connect(MONGO_URI).catch(e => console.error('Retry failed:', e));
  }, 5000);
});
