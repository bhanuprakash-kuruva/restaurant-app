...
const SalesAndItemsAnalysis = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('https://restaurant-app-backend-mu.vercel.app//orders/deliveryboy/showorders');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        const response = await fetch('https://restaurant-app-backend-mu.vercel.app//reviews/showreviews'); // Replace with your API endpoint for ratings
        if (!response.ok) {
