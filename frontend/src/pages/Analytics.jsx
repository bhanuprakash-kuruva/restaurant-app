...
const SalesAndItemsAnalysis = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://localhost:8071/orders/deliveryboy/showorders');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        const response = await fetch('http://localhost:8071/reviews/showreviews'); // Replace with your API endpoint for ratings
        if (!response.ok) {
