...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserDetails = async () => {
      try {
        const response = await fetch(`http://localhost:8011/customer/profile/${e}`);
        if (!response.ok) {
  ...
  const handleRemoveFromWishlist = async (itemId) => {
    try {
      ...
      const response = await fetch(`http://localhost:8011/customer/remove-wishlist-item/${email}`, {
        method: 'PUT',
  ...
  const handleSaveChanges = async () => {
    try {
      const response = await fetch(`http://localhost:8011/customer/profile/update/${email}`, {
        method: 'PUT',
        headers: {
